...
const _lastAskForReviewKey = 'last_review_request_date';
const _hasRateAppKey = 'has_rate_app';
const _hasReviewAppKey = 'has_review_app';
...
class RatingApi {
  ...
  Future<void> saveHasReviewApp() async {
    await _prefs.setBool(_hasReviewAppKey, true);
  }

  Future<bool> hasReviewApp() async {
    final hasReviewApp = _prefs.getBool(_hasReviewAppKey);
    return hasReviewApp ?? false;
  }

  Future<void> saveLastReviewAskingDate() async {
    final now = DateTime.now();
    await _prefs.setInt(_lastAskForReviewKey, now.millisecondsSinceEpoch);
  }

  Future<DateTime?> lastReviewAskingDate() async {
    final lastAskingDate = _prefs.getInt(_lastAskForReviewKey);
    if (lastAskingDate == null) {
      return null;
    }
    return DateTime.fromMillisecondsSinceEpoch(lastAskingDate);
