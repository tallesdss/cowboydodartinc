...
async function isSignatureValid(rawQuery: string): Promise<boolean> {
  ...
  if (!key) {
    error(`[ads-ssv] no verifier key for key_id=${keyId}`);
    return false;
  }
  ...
  async (req: https.Request, res: express.Response) => {
    ...
    try {
      ...
      try {
        ...
        error("[ads-ssv] verification error", e);
      ...
      if (!valid) {
        ...
        info("[ads-ssv] invalid signature — dropping");
      ...
      const rewardAmount = Number(req.query.reward_amount ?? 0);
      const rewardItem = (req.query.reward_item as string | undefined) ?? "";
      const customData = (req.query.custom_data as string | undefined) ?? "";
      const adUnit = (req.query.ad_unit as string | undefined) ?? "";
      ...
      if (!userId || !transactionId) {
        ...
        info("[ads-ssv] valid callback without user_id/transaction_id");
        res.status(200).send("ok");
        return;
      }
      ...
      await db.runTransaction(async (tx) => {
        ...
        if (existing.exists) return; // already granted: idempotent
        tx.set(rewardRef, {
          amount: rewardAmount,
          item: rewardItem,
          customData,
      ...
      info(`[ads-ssv] reward granted user=${userId} tx=${transactionId}`);
      res.status(200).send("ok");
    } catch (e) {
      error("[ads-ssv]", e);
